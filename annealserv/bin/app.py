...
#     computedRates = 0
    ...
    def POST(self):
        ...
        if len(form.sequence) > MAX_SEQ_LEN or len(form.sequence) < 3 :
            return render.errorpage(greeting=form.sequence, substrate=form.substrate, result=1e-36)
...
#             print "Rates computes = " + str(self.computedRates) + "\n"
            ...
            print str(e)
            return render.errorpage(greeting=form.sequence, substrate="Exception", result=1e-36)

        if float(float(form.result['rate'])) < 10.0 ** -30:
            return render.errorpage(greeting=form.sequence, substrate=form.substrate, result=1e-36)
        
        return render.index(greeting=form.sequence, substrate=form.substrate, result=form.result)
